...
@pytest.mark.skipif(
    not (
        os.path.exists("data/processed/train_processed.pt")
        or os.path.exists("data/processed/test_processed.pt")
    ),
...
def test_Classes_Represented():
    ...
    Test = torch.load("data/processed/test_processed.pt")
    assert (
        torch.unique(Train[:][1]).numpy() == np.arange(0, 10)
    ).all(), "All classes not represented in train data"
    assert (
        torch.unique(Test[:][1]).numpy() == np.arange(0, 10)
    ).all(), "All classes not represented in test data"


@pytest.mark.skipif(
    not os.path.exists(f"{_PATH_DATA}/processed/test_processed.pt"), reason="Data files not found"
)
def test_DataShape():
    Train = torch.load("data/processed/train_processed.pt")
    Test = torch.load("data/processed/test_processed.pt")
